...
class CustomUserManager(BaseUserManager):
      def create_user(self, email, firs_name, last_name, password=None, **extra_fields):
            if not email:
                  raise ValueError('The Email field must be set.')
            email = self.normalize_email(email)
            user = self.model(email=email, first_name=firs_name, last_name=last_name, **extra_fields)
