    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.headers.get('HX-Request'):
                return HttpResponse(headers={'HX-Redirext': reverse('users:profile')})
...
def update_account_details(request):          
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            user.clean()
            user.save()
            update_user = CustomUser.objects.get(id=user.id)
            request.user = update_user
            if request.headers.get('HR-Request'):
                return TemplateResponse(request, 'user/partials/account_details.html', {'user': update_user})
            return TemplateResponse(request, 'users/partials/account_details.html', {'user': update_user})
        else:
            return TemplateResponse(request, 'users/partials/edit_account_details.html', 
                                    {'user': request.user, 'form': form})
        
    if request.headers.get('HX-Request'):
        return HttpResponse(headerts={'HX-Redirect': reverse('user:profile')})
    return redirect('users:profile')
